...
- The dichotomy and hclust sometimes produce (on average) close to the desired size but many cluster are very much smaller/bigger.
...
That's why the nearest neighbors approach with the *maxD* rule, which starts with outlier points on purpose, is performing better with a narrower distribution and much weaker outliers.
...
After implementing the loop using Rcpp, I ran it on datasets of different size and compare the computation times.
...
st.df %>% select(-system) %>% spread(method, elapsed) %>% ggplot(aes(x=n, y=R/Rcpp)) +
  geom_point() + theme_bw() + xlab('points')
```

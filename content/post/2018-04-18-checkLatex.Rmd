...
To continue on the series of *"Things I did instead of writing my thesis to help me write my thesis"*, another Python script reads a LaTeX manuscript and helps checking that everything is fine.
...
1. Find multi-references to reorder.
...
1. Shows the first usage of an acronym (not in italic/serif).

This was particularly useful for my thesis because I had to combine three manuscripts plus a general intro that shared some references. 
I had to make sure that the labels, acronyms, references were all correct and consistent throughout the thesis.
I might use it again on smaller documents like research manuscripts.

## Why this output?

"*List missing references*" is redundant with `bibtex` output but this output was sandwiched between long latex logs.
"*Find multi-references to reorder*" is to transform "something<sup>19,3,12</sup>" into "something<sup>3,12,19</sup>". 
...
"*List figures/tables that are not in order*" to make sure the figure numbers are in ascending order in the text. 
...
"*List acronyms in serif (for method names)*" to make sure that acronyms in serif are methods.

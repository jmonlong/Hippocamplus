...
- [*wdl-mode*](https://github.com/zhanxw/wdl-mode) written by [Xiaowei Zhan](http://zhanxw.com/) (install through MELPA).
- Change the indentation size with `(setq wdl-indent-level 4)` in the `.emacs` file.
- Also `(setq-default indent-tabs-mode nil)` to make sure tabs are made of spaces (WDL doesn't like mixing spaces and tabs).

I also use [polymode](https://polymode.github.io/) to swith to "shell" mode within *command* chunks.
Once *polymode* is installed (e.g. though MELPA), I added to my `.emacs`:

~~~lisp
(require 'polymode)
(define-hostmode poly-wdl-hostmode
  :mode 'wdl-mode)
(define-innermode poly-wdl-command-innermode
  :mode 'sh-mode
  :head-matcher "^[ \t]*command *{ *\n"
  :tail-matcher "^[ \t]*} *\n"
  :head-mode 'host
  :tail-mode 'host)
(define-innermode poly-wdl-command2-innermode
  :mode 'sh-mode
  :head-matcher "^[ \t]*command *<<< *\n"
  :tail-matcher "^[ \t]*>>> *\n"
  :head-mode 'host
...
(define-polymode poly-wdl-mode
  ...
  :innermodes '(poly-wdl-command-innermode
		poly-wdl-command2-innermode))

...
cit.n = pm.df %>% filter(!grepl('Review', type)) %>%
  select(title, year, cited, pmid, date, journal) %>% unique %>% 
...
figure(legend_location=NULL, width=800,height=500,tools="reset") %>%
  ly_points(date, cited, data = cit.n, color=year.rank,
            hover=list(title, year, journal, cited)) %>%
...
```{r}
pub.df = pm.df %>% filter(hgen) %>% group_by(author) %>%
  summarize(pubs=n(), firstDate=min(date), NatScCell=sum(big)) %>%
  filter(pubs>2) %>% arrange(NatScCell)

figure(legend_location=NULL, width=800,height=500,tools="reset") %>%
  ly_points(firstDate, pubs, data = pub.df,
            hover=list(author, pubs, NatScCell)) %>%
  tool_pan %>% tool_wheel_zoom %>% y_axis(log = TRUE)
...
pm.df %>% filter(hgen, aff.single, aff.nchar<400) %>%
  group_by(title, pmid, nb.auth) %>%
  summarize(nb.hgen=n(), prop.hgen=nb.hgen/nb.auth[1],
            hgen.score=nb.hgen - (nb.auth[1]-nb.hgen),
            authors=paste(author, collapse=",")) %>%
  ungroup %>% select(-nb.auth) %>% arrange(desc(hgen.score), desc(prop.hgen)) %>%
    head %>% urlpm %>% kable(digits=2)
```	
...
```{r}
gens = lapply(unique(fam.df$mentor), function(ment){
...
big.fams = gens.df %>% arrange(desc(gens)) %>% head(30) %>% .$lineage %>% strsplit('->') %>%
  unlist
fam.nw = fam.df %>% filter(mentor %in% big.fams)
...
It looks like PubMed records "changed" around 2003. 
For example the author names changed from *Initials LastName* to *FirstName LastName*. 
That causes the stats for some authors to be split in two. 
It also means that the "department growth" is biased.
Maybe PubMed changed in 2003, for example increasing the list of journals/databases that it includes, which then looks like a boost of publications/authors in the department.
